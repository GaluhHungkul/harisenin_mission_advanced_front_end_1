...
const Navbar = () => {
  ...
  return (
    ...
    {!disableNavbar.includes(location.pathname) && <nav className="bg-primary sticky top-0 z-50 flex items-center justify-between px-5 py-2 h-14 lg:h-24 lg:px-20 lg:py-6 ">
        <div className="flex lg:justify-between border">
            <Link to="/" className="mr-4">
                <img src={logoMobile} alt="" className="lg:hidden" />
                <img src={logoDesktop} alt="" className="hidden lg:block"/>
            </Link>
            <ul className="flex gap-3 text-white justify-end items-center w-[268px] text-[10px] lg:w-[585px] lg:gap-20 lg:text-[18px]">
        ...
        <div className="relative items-center flex gap-4 cursor-pointer lg:scal0 border border-white" onClick={() => setShowProfile(!showProfile)}>
            <img src={avatar} alt="" className=""/>            
            <FontAwesomeIcon icon={faChevronDown} className={`${showProfile && "rotate-180"} duration-300 `} color="#fff" />

        </div>
        {showProfile &&
            <div className="fixed text-white  p-3 right-4 top-12 rounded border border-slate-700 w-max gap-2 flex flex-col bg-primary text-[10px] lg:text-[18px]">
                {profileNavigate.map((item, index) => (
                    <Link to={item.link} key={index} className="flex items-center hover:text-blue-500">
